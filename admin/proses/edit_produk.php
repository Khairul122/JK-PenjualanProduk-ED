...
if($eror === 4){
    ...
    $result = mysqli_query($conn, "UPDATE produk SET nama = '$nm_produk', deskripsi = '$desk', harga = '$harga', status = '$status' where kode_produk = '$kode'"); // Tambahkan status
...
if (file_exists("../../image/produk/".$tgambar['image'])) {
    ...
    $result = mysqli_query($conn, "UPDATE produk SET nama = '$nm_produk', image = '$namaGambarBaru' ,deskripsi = '$desk', harga = '$harga', status = '$status' where kode_produk = '$kode'"); // Tambahkan status
